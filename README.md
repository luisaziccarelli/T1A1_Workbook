...
Secondly, although the second line appears to be correct at a first glance, it would not be properly executed given there are errors in our first line of code. If Ruby assumes the value of Celsius is a string ```”30”``` then in line two it would try to multiply ```”30” x 9```, which would return ```"303030303030303030"```, instead of ```270```. Then the program would break because Ruby would not be able to perform a mathematical function like a division with a string. 
